...
export class AuthController {
    ...
    @Throttle({default: {limit: 5, ttl: 900}}) @Post('send-otp') sendOtp(@Body() dto: SendOtpDto) {
        return this.auth.sendOtp(dto.email);
    }

    @Post('verify-otp') async verifyOtp(@Body() dto: VerifyOtpDto, @Res({passthrough: true}) res: Response,) {
    ...
    @UseGuards(JwtAuthGuard) @Post('logout') async logout(@Req() req: {
        user: AuthUser
    ...
    @UseGuards(JwtAuthGuard) @Post('me') me(@Req() req: { user: AuthUser }) {
        return req.user;
    }
}